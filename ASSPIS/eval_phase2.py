...
def run_tune(args: argparse.Namespace) -> int:
    ...
    print(f"[Baseline@tune] phase3_score={base_score:.4f} | wmape_weighted={base_dm.get('wmape_weighted'):.4f} | peak_std={base_dm.get('wmape_peak_11_12',{}).get('std'):.4f} | dec_mean={base_dm.get('wmape_dec',{}).get('mean'):.4f}")
    ...
    if not getattr(args, "skip_optuna", False):
        ...
        def objective(trial: "optuna.Trial") -> float:
            params = {
                ...
                "n_estimators": int(trial.suggest_int("n_estimators", 300, 2000, step=50)),
            ...
            dm = _score_rows(rows)
            score = float(dm.get("phase3_score", float("inf")))
            if score != score:
                score = float("inf")
        ...
        study = optuna.create_study(
            study_name="sales_forecast_study",
            storage="sqlite:///optuna_journal.db",
            load_if_exists=True,
            directions=["minimize", "minimize"]
        )
        study.optimize(objective, n_trials=int(getattr(args, "optuna_trials", 30)))

        best_optuna_params = dict(study.best_params)
        best_optuna_score = float(study.best_value)
        print(f"[Optuna] best phase3_score={best_optuna_score:.4f}")

        opt_dir = out_dir / "optuna"
        opt_dir.mkdir(parents=True, exist_ok=True)
        _write_json(opt_dir / "best_params.json", best_optuna_params)
        _write_json(opt_dir / "best_value.json", {"phase3_score": best_optuna_score})
        try:
            df_trials = study.trials_dataframe()
            df_trials.to_csv(opt_dir / "trials.csv", index=False, encoding="utf-8-sig")
        except Exception:
            pass

    # -------------------------
    # Random Search baseline (KPI-consistent)
    # -------------------------
    ...
    if not getattr(args, "skip_random", False):
        ...
        def sample_one() -> dict:
            ...
            return {
                ...
                "n_estimators": int(rng.integers(300, 2001) // 50 * 50),
        ...
        for i in range(n_iter):
            ...
            rand_trials.append({
                "iter": i,
                "phase3_score": score,
                "wmape_weighted": dm.get("wmape_weighted"),
                "peak_std_11_12": (dm.get("wmape_peak_11_12") or {}).get("std"),
                "dec_mean": (dm.get("wmape_dec") or {}).get("mean"),
                **params,
            })
            ...
            if (i + 1) % max(1, n_iter // 10) == 0:
                print(f"[Random] {i+1}/{n_iter} best_score={best_score:.4f}")

        best_rand_params = dict(best_params or {})
        best_rand_score = float(best_score)
        print(f"[Random] best phase3_score={best_rand_score:.4f}")

        rnd_dir = out_dir / "random"
        rnd_dir.mkdir(parents=True, exist_ok=True)
        _write_json(rnd_dir / "best_params.json", best_rand_params)
        _write_json(rnd_dir / "best_value.json", {"phase3_score": best_rand_score})
        if rand_trials:
            pd.DataFrame(rand_trials).sort_values("phase3_score").to_csv(rnd_dir / "trials.csv", index=False, encoding="utf-8-sig")
    ...
    base_dir = _export("baseline_full", None)

    opt_dir = None
    rnd_dir = None
    if best_optuna_params is not None:
        opt_dir = _export("optuna_best_full", {**base_params, **best_optuna_params})
    if best_rand_params is not None:
        rnd_dir = _export("random_best_full", {**base_params, **best_rand_params})
