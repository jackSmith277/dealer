...
AI_API_KEY = "sk-gvlleYSWqm5LRvfQ8e6744F1802d4d9b8aEf1d400800443d"
AI_MODEL = "Qwen/Qwen2.5-7B-Instruct"
AI_SYSTEM_PROMPT = """你是一名专业的 4S 店汽车销售顾问。你的任务是协助销售人员进行客户接待、车型介绍、价格计算和邀约试驾。
核心能力：
熟悉店内所有车型的配置、价格、优惠政策。
...
def api_chat():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return jsonify({'ok': False, 'error': f'AI服务错误: {response.status_code}'}), 500
...
def api_crawl_comments():
    ...
    try:
        ...
        if not url.strip():
            return jsonify({'ok': False, 'error': 'URL不能为空'}), 400
        
        try:
            from 新爬取 import crawl_autohome_comments
            comments = crawl_autohome_comments(url, max_items)
            ...
            return jsonify({'ok': False, 'error': '爬取模块未安装'}), 500
            
    except Exception as e:
        print(f'爬取评论错误: {str(e)}')
        return jsonify({'ok': False, 'error': str(e)}), 500
