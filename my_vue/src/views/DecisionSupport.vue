  ...
  methods: {
    checkUserRole() {
      ...
      if (this.isAdmin) {
        this.tableColumns = [
          { key: 'region', label: '区域' },
          { key: 'dealerCount', label: '经销商数量' },
          { key: 'totalSales', label: '总销售额' },
          { key: 'totalOrders', label: '总订单量' },
          { key: 'avgProfit', label: '平均利润' },
          { key: 'policyCoverage', label: '政策覆盖率' },
          { key: 'executionScore', label: '执行效果评分' }
        ]
      } else {
        this.tableColumns = [
    ...
    getFilteredDealerData() {
      ...
      if (this.filters.dealerCode) {
        ...
        dealers = this.dealerData.filter(d => d['省份'] === this.filters.province)
      } else if (this.filters.region) {
        dealers = this.dealerData.filter(d => this.filteredProvinces.includes(d['省份']))
      } else {
    ...
    async loadTableData() {
      try {
        ...
        const response = await axios.get('/api/decision/table-data', { params })
    ...
    viewAdviceDetail(advice) {
      this.$message.info(`查看建议详情: ${advice.title}`)
    },
    
    implementAdvice(advice) {
      this.$message.success(`开始执行建议: ${advice.title}`)
    },
    ...
    formatTableCell(value, key) {
      ...
      if (['totalSales', 'avgProfit', 'sales', 'profit'].includes(key)) {
        return this.formatNumber(value)
      }
      
      if (['policyCoverage', 'executionScore', 'satisfaction'].includes(key)) {
        return `${value}%`
      }
